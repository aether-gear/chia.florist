...
Chia Florist uses **automated independent semantic versioning and release management** powered by **GitHub Actions**.

Versioning is **independent per module** — modifying one service will only trigger version bumps, GitHub Releases, and Docker image tags for that specific module, with **zero bot-authored commits** and **zero release pull requests**.
...
GitHub Actions (Release Workflow)
        ↓
Determine which packages changed (Path-based)
        ↓
Calculate independent version bumps
        ↓
Create Git tags & GitHub Releases directly
        ↓
Downstream CI builds trigger on tag (Docker / GHCR)
        ↓
Done (Clean Git history, no bot commits)
```

---

## 2. Commit Trigger Words Cheatsheet

The release workflow inspects the prefix and footer of commits on `main` touching each package path to determine the version bump:

| Bump Type | Version Example | Commit Trigger Prefix | Description & Examples |
| :--- | :--- | :--- | :--- |
| **PATCH** | `0.15.0` → `0.15.1` | `fix:`<br>`perf:` | **Bug fixes & Performance enhancements**<br>• `fix: resolve custom product leak to cart`<br>• `fix(auth): isolate session state on logout`<br>• `perf(db): add index on order_item_custom_designs` |
| **MINOR** | `0.15.0` → `0.16.0` | `feat:` | **New backward-compatible features**<br>• `feat: introduce multi-courier tracking`<br>• `feat(orders): implement automatic refund on cancel`<br>• `feat(ai): add live SKU forecaster` |
...
> [!NOTE]
> **Pre-1.0 (`0.x.y`) Mode**: Because `"bumpMinorPreMajor": true` is enabled in [.github/release-config.json](file:///d:/__Projects/kage/chia.florist/.github/release-config.json), breaking changes during pre-1.0 development bump the **minor** version (`0.15.0` → `0.16.0`) to avoid inflating major version numbers before domain contracts stabilize for `1.0.0`.

---

## 3. How Monorepo Detection & Overlapping Commits Work

> [!IMPORTANT]
> **The release engine tracks modified FILE PATHS, NOT the commit scope name.**
...
Write the commit based on the feature/domain. The release engine checks which directory the changed files live in:

```bash
# Modifies files in service-core/ -> Bumps service-core (0.15.0 -> 0.16.0)
...
# Modifies files in e-commerce/ -> Bumps e-commerce (0.9.0 -> 0.9.1)
...
# Modifies files in control-panel/ -> Bumps control-panel (0.6.0 -> 0.7.0)
...
# Modifies files in intelligence-layer/ -> Bumps intelligence-layer (0.4.0 -> 0.5.0)
...
When a feature touches files across multiple directories in one commit or PR, **the release engine automatically detects all affected modules and bumps each one independently**:
...
# -> Result: Bumps BOTH service-core (0.15.0 -> 0.16.0) AND e-commerce (0.9.0 -> 0.10.0)
# -> Generates GitHub Release notes for both tags.
...
## 4. Current Module Baseline Versions

| Module | Directory | Tag Format | Docker Image |
| :--- | :--- | :--- | :--- |
| **`service-core`** | `service-core/` | `service-core-v0.15.0` | `ghcr.io/aether-gear/chia.florist/service-core:0.15.0` |
| **`e-commerce`** | `e-commerce/` | `e-commerce-v0.9.0` | `ghcr.io/aether-gear/chia.florist/e-commerce:0.9.0` |
| **`control-panel`** | `control-panel/` | `control-panel-v0.6.0` | Static Netlify build |
| **`intelligence-layer`** | `intelligence-layer/` | `intelligence-layer-v0.4.0` | `ghcr.io/aether-gear/chia.florist/intelligence-layer:0.4.0` |

---

## 5. Custom Product Contract & Schema Versioning
...
* **Editor Engine Version**: `3.1.0` (Defined in [e-commerce/app/features/custom-product/constants.ts](file:///d:/__Projects/kage/chia.florist/e-commerce/app/features/custom-product/constants.ts))
