...
type AuditLog struct {
	...
	// ActorID is the authenticated user or service that performed the action.
	// Empty when the action was performed by an unauthenticated request.
	ActorID string

	Outcome AuditOutcome

	// Correlation fields — populated from the request context.
	RequestID string
	ClientIP  string

	// Metadata holds arbitrary structured data specific to the event.
	Metadata map[string]any

	CreatedAt time.Time
}

// AuditLogRepository is the persistence interface for audit log records.
// Defined in the domain so the infra layer depends inward, not outward.
