...
func NewProductImageUploadRepository(
	fileStore infraStorage.Provider,
	imageGen sharedImage.VariantCreator,
) repository.ProductImageUploadRepository {
...
func (s *productImageUploadService) UploadCatalogImage(
	params repository.UploadProductImageParams,
) (string, error) {
	return s.uploadVariant(
		params.ProductID,
		params.Metadata,
		params.File,
...
func (s *productImageUploadService) UploadCartImage(
	params repository.UploadProductImageParams,
) (string, error) {
	return s.uploadVariant(
		params.ProductID,
...
func (s *productImageUploadService) UploadDetailImage(
	params repository.UploadProductImageParams,
) (string, error) {
	return s.uploadVariant(
		params.ProductID,
...
func (s *productImageUploadService) DeleteUploadedImages(
	urls []string,
) error {
	var lastErr error
	for _, u := range urls {
		if err := s.fileStore.Delete(u); err != nil {
...
func (s *productImageUploadService) uploadVariant(
	productID uuid.UUID,
	...
	resizedStream, size, err := s.imageGen.GenerateVariant(
		file,
	...
	object, err := s.fileStore.Upload(infraStorage.UploadInput{
		Key:           objectKey,
		File:          resizedStream,
