...
func (u *CheckoutUsecase) Execute(
	...
	for _, shopGroup := range input.ShopInput {
		...
		if err != nil {
			// Fallback gracefully to mock options based on the shop's active couriers
			// if the shipping API daily limit is exceeded or if there is any other error.
			costOptions = make([]shipmentRepo.CostOption, 0, len(couriers))
			for _, courier := range couriers {
				var name, service, etd string
				var cost int64
				...
				costOptions = append(costOptions, shipmentRepo.CostOption{
					Code:        courier.Code,
					Name:        name,
					Service:     service,
